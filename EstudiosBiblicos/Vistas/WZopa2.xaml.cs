...
namespace EstudiosBiblicos.Vistas
{
    ...
    public partial class WZopa2 : ContentPage
    {
        // word manager object
        ...
        public WZopa2()
        {
            ...
            FlexScoreHeader.WidthRequest  = App._width;
            FlexScoreHeader.HeightRequest = App._height;
            Preferences.Set("soundOn", false);
            var listado = App.Database.GetPalabra(App.LeccionSeleccionada.IdLeccion);
            lblenc.Text = listado.Texto1;
            lbldet.Text = listado.Texto2;
            Rows = 0;
            Columns = 0;
            InitalizeHeaderAndTiles(App._width, App._height);
        }
        
        // Initialize tiles
        private bool InitalizeHeaderAndTiles(double width, double height)
        {
            bool bOK = true;
            try
            {
                // Check if sound is on
                SoundSettingIsOn = Preferences.Get("soundOn", false);
                ...
                ScreenWidth = width;
                ScreenHeight = height;
                TilesScreenHeight = height - App.HEADER_HTML_HEIGHT;
